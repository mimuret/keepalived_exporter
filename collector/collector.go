...
var proto2string = map[int]string{
...
func NewKACollector(useJSON bool, sigJSON int) (*KACollector, error) {
	...
	if handle, err := ipvs.New(""); err == nil {
		labelsLVS := []string{"service", "addr", "proto"}
...
func (k *KACollector) Collect(ch chan<- prometheus.Metric) {
	...
	for _, s := range svcs {
		...
		proto, _ := proto2string[int(s.Protocol)]
		service := proto + ":/" + addr
